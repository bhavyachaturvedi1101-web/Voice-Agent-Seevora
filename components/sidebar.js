...
export function renderSidebar(activeRoute, user) {
  ...
  const userInitials = user?.initials || 'A';
  const userEmail = user?.email || 'admin@seevora.com';

  return `
    <nav class="side-nav" id="main-sidebar">
      <div class="side-nav-header" style="height: 100px; padding-left: 32px;">
        <div class="logo">
          <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor" color="#3b82f6"><path d="M12 2v20M16 6v12M8 6v12M20 9v6M4 9v6" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"/></svg>
      ...
      <div style="padding: 24px; border-top: 1px solid var(--border-subtle);">
        ...
        <div style="display: flex; align-items: center; gap: 12px;">
          <div class="avatar" style="width: 36px; height: 36px; background: #e0e7ff; color: #3730a3; font-size: 0.85rem; box-shadow: none;">${userInitials}</div>
          <div style="flex: 1; overflow: hidden; cursor: default;">
            <div style="font-size: 0.9rem; font-weight: 600; color: var(--text-primary);">${userName}</div>
            <div style="font-size: 0.75rem; color: var(--text-muted); text-overflow: ellipsis; overflow: hidden; white-space: nowrap;">${userEmail}</div>
          </div>
